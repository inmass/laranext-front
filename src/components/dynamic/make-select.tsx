...
const MakeSelect = forwardRef<SelectRef, MakeSelectProps>(
  ({ value, onChange, onBlur, className, disabled, name }, ref) => {
    const t = useTranslations(
      'Dashboard.CarListings.Wizard.steps.BasicInfoStep.MakeSelect'
    );
    ...
    const generateImageElement = (
      ...
      return (
        <Image src={getMakeImage(slug)} alt={make} width={30} height={30} />
      );
    };

    const options: Option[] =
      data?.data?.map(({ id, name, slug }: Make) => ({
        label: name,
        value: String(id),
        labelImage: generateImageElement(name, slug),
      })) || [];
