...
const CarListings = () => {
  ...
  const [params, setParams] = useState<CarListingsParams>({
    page: 1,
    perPage: 10,
    sort: { key: 'created_at', direction: 'desc' } as {
      key: string;
      direction: 'asc' | 'desc';
    },
  ...
  const columns = [
    {
      header: '',
      type: 'image' as const,
      accessor: (item: CarListingType) => item.primary_image?.path,
      className: 'hidden md:table-cell md:w-1/12',
    },
    {
      header: t('columns.user'),
      accessor: (item: CarListingType) => item.user?.name,
      className: 'table-cell',
      filterable: true,
      filterType: 'text' as const,
      filterParam: 'user.name' as const,
    },
    {
    ...
    {
      header: t('columns.isSold'),
      accessor: 'is_sold' as const,
  ...
  const actions = [
    ...
    {
      name: t('actions.isSold'),
      accessor: (item: CarListingType) => (
        <AlertDialog
          trigger={
            <CircleDot className="h-5 w-5 cursor-pointer text-yellow-500" />
          }
          description={t(
            item.is_sold
              ? 'isSoldConfirmation.description.toSold'
              : 'isSoldConfirmation.description.toNotSold'
          )}
  ...
  const handleParamsChange = useCallback(
    (newParams: Partial<CarListingsParams>) => {
