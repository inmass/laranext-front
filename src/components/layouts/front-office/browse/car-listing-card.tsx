...
const CarListingCard: React.FC<CarListingCardProps> = ({
  ...
  return (
    <div
      key={carListing.id}
      className={cn(
        'bg-muted/40 rounded-lg border border-border col-span-1 hover:border-border/50 transition-all duration-300 flex flex-col relative',
        className
      )}
    >
      {carListing.is_sold && (
      ...
      />
      <Link
        href={AppRoutes.frontOffice.listing(carListing.slug)}
