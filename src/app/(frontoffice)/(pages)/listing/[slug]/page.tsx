      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="relative">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            {true && (
              <>
                <DetailItem
                  ...
                  value={carListing.original_price}
                />
                <br />
              </>
            )}
            ...
            <DetailItem
              label={t('mileage')}
              value={`${carListing.mileage} km`}
            />
            <DetailItem
              label={t('transmission')}
              value={translateTransmissionType(
            ...
            <DetailItem
              label={t('fuelType')}
              value={translateFuelType(carListing.fuel_type, tFuelTypes)}
            />
            <DetailItem
              label={t('bodyStyle')}
              value={carListing.body_style?.name}
            />
            <DetailItem
              label={t('condition')}
              value={translateConditionType(
                carListing.condition?.name,
                tConditionTypes
              )}
            />
            <DetailItem
              ...
              value={
                <div className="flex items-center">
                  <div
                    className="w-7 h-7 rounded-full mt-2 mr-2 p-2 border-2 border-muted"
                    style={{ backgroundColor: carListing.exterior_color }}
                  ></div>
                </div>
              }
            />
            <DetailItem
              ...
              value={
                <div className="flex items-center">
                  <div
                    className="w-7 h-7 rounded-full mt-2 mr-2 p-2 border-2 border-muted"
                    style={{ backgroundColor: carListing.interior_color }}
                  ></div>
                </div>
              }
            />
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {carListing.user?.name || t('anonymousSeller')}
                  </p>
                </div>
              </div>
          ...
          {Object.values(groupedFeatures).map((group: any, index: any) => (
            <div key={index}>
              <h3 className="text-lg font-medium mb-2">
                {translateFeatureType(group.name, tFeatureTypes)}
              </h3>
...
const DetailItem = ({
  ...
  <div>
    <p className="text-sm text-muted-foreground">{label}</p>
    {
      ...
      typeof value === 'object' ? value : <p className="font-medium">{value}</p>
    }
  </div>
);
