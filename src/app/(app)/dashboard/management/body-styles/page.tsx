...
import { BodyStylesParams, getBodyStyles, useDeleteBodyStyle } from '@/hooks/api/body-styles';
...
const BodyStyles = () => {
    ...
    const actions = [
        { name: t('actions.edit'), accessor: (item: BodyStyleType) => (
                <BodyStyleFormDialog bodyStyle={item} />
            )
        },
        { name: t('actions.delete'), accessor: (item: BodyStyleType) => (
            <AlertDialog
                trigger={
    ...
    const handleParamsChange = useCallback((newParams: Partial<BodyStylesParams>) => {
