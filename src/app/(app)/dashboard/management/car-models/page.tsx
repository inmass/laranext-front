...
const CarModels = () => {
  ...
  const columns = [
    ...
    { header: t('columns.slug'), accessor: 'slug' as const },
    {
      ...
      accessor: (item: CarModelType) => item.make?.name,
      filterable: true,
      filterType: 'text' as const,
      filterParam: 'make.name' as const,
    },
    {
  ...
  const actions = [
    {
      name: t('actions.edit'),
      accessor: (item: CarModelType) => <CarModelFormDialog carModel={item} />,
    },
    {
      name: t('actions.delete'),
      accessor: (item: CarModelType) => (
        <AlertDialog
  ...
  const handleParamsChange = useCallback(
    (newParams: Partial<CarModelsParams>) => {
