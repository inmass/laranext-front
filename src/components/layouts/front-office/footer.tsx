    ...
    <footer className="bg-foreground text-background py-8 ">
      <div className="container mx-auto px-10">
        <div className="text-center md:text-left grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div>
            <h3 className="text-lg mb-4 text-gray-500 font-bold">{t('explore')}</h3>
            <ul className="space-y-2">
              <li><Link href={AppRoutes.frontOffice.browse}>{t('browse')}</Link></li>
              <li><Link href={AppRoutes.frontOffice.about}>{t('aboutUs')}</Link></li>
              <li><Link href={AppRoutes.frontOffice.contact}>{t('contactUs')}</Link></li>
              <li><Link href={AppRoutes.frontOffice.privacyPolicy}>{t('privacyPolicy')}</Link></li>
            </ul>
          </div>
          
          <div>
            <h3 className="text-lg mb-4 text-gray-500 font-bold">{t('contactUs')}</h3>
            <Link href={`mailto:${t('contact.email')}`}>klasikiat@gmail.com</Link><br />
        ...
        <div className="mt-10 flex flex-col md:flex-row justify-between items-center text-sm">
          <p>{t('copyright', { year: new Date().getFullYear(), appName: getAppName() })}</p>
