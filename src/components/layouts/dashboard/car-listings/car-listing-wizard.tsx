...
const addCarListingSchema = (t: (key: string) => string) =>
  z.object({
    make_id: z.string().min(1, { message: t('validation.makeRequired') }),
    car_model_id: z.string().min(1, { message: t('validation.modelRequired') }),
    body_style_id: z
      .string()
      .min(1, { message: t('validation.bodyStyleRequired') }),
    condition_id: z
      .string()
      .min(1, { message: t('validation.conditionRequired') }),
    features: z
      .array(z.string())
      .min(1, { message: t('validation.featuresRequired') }),
    ...
    exterior_color: z
      .string()
      .min(1, { message: t('validation.exteriorColorRequired') }),
    interior_color: z
      .string()
      .min(1, { message: t('validation.interiorColorRequired') }),
    transmission: z
      .string()
      .min(1, { message: t('validation.transmissionRequired') }),
...
const CarListingWizard = ({
  onSubmitSuccess,
  carListing,
}: CarListingWizardProps) => {
  ...
  const {
    handleSubmit,
    trigger,
    formState: { isDirty, isValid: formIsValid },
  } = methods;
  ...
  const submitForm = async (data: CarListingFormData) => {
    try {
      if (carListing) {
        await updateCarListingMutation.mutateAsync({
          ...data,
          id: carListing.id,
        });
      } else {
    ...
    <LookupProvider>
      <FormProvider {...methods}>
        <form>
          ...
          >
            <Tabs.List
              className="flex space-x-1 rounded-xl bg-muted p-2 flex-wrap justify-center"
                ...
                <Tabs.Trigger
                  ...
                  className={cn(
                    ...
                    index === currentStep
                      ...
                      : 'bg-muted',
                    !(!canSkipSteps && index > currentStep) &&
                      'hover:bg-muted-foreground hover:text-card',
                    index >= 5 && 'mt-1'
                  )}
                >
                  {step.title}
...
export function mapCarListingForFormData(
  carListing: CarListingType
): UpdateCarListingFormData {
  return {
    id: carListing.id,
