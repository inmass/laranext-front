...
const Conditions = () => {
    ...
    const actions = [
        { name: t('actions.edit'), accessor: (item: ConditionType) => (
                <ConditionFormDialog condition={item} />
            )
        },
        { name: t('actions.delete'), accessor: (item: ConditionType) => (
            <AlertDialog
                trigger={
    ...
    const handleParamsChange = useCallback((newParams: Partial<ConditionsParams>) => {
